...
- **CUOTA 40.000:** ~~confirmar qué representan esas filas~~ **resuelta, sin acción pendiente.** El cliente ya aclaró (2026-07-10) que la mensualidad es **$50.000 para todos** y que el descuento de hermanos aplica al uniforme (R2/R9). El seed **nunca lee la columna CUOTA**: fuerza `CUOTA = 50000` (`scripts/seed-from-excel.mjs:24`), así que los 40.000 del Excel son un dato viejo de la hoja que no entra a la base ni afecta ningún cálculo.
- **Año de nacimiento 2024 (un alumno):** ~~fuera de todo rango de categoría~~ **resuelta** — el cliente confirmó (2026-07-27) que es **2014**, no 2024. El Excel actualizado ya trae `2014-07-16` y el seed del 2026-07-30 lo dejó en `SUB 12`. `scripts/fix-anio-miguel-angel.mjs` queda como red de seguridad idempotente.
- **`KEINER BOSCAN` en SUB 14 vs SUB 16:** ~~conflicto de categoría~~ **resuelta, no era un dato malo** — nació el `2011-12-15`, así que por **edad cumplida** tiene 14 años y SUB 14 es correcto; solo el fallback **por año** daba SUB 16. El seed comparaba la categoría del Excel contra la del año aunque la fila trajera fecha completa, y por eso descartaba a este alumno. Corregido en `scripts/seed-filas.mjs`: el cross-check vale contra **cualquiera** de las dos reglas.
- **Filas que el seed omitía (3): quedan 1.** Verificado contra la hoja `CATEGORIAS` el 2026-08-22: `GERONIMO ESCORCIA` ya trae `2016` en `F13` (el `2106` está corregido; el cliente reconfirmó la fecha `08/09/2016`, que además ya coincidía con la base) y `ABRAHAM PEREZ` ya trae `2018-01-05`. **Sigue vigente solo `JUAN PABLO MAESTRE`** (fila 78, columna de documento vacía) → el seed lo seguirá omitiendo hasta que se le ponga documento en la hoja.
- **Los que salieron de la hoja `CATEGORIAS`:** ~~confirmar si se retiraron~~ **resuelta (cliente, 2026-08-07)** — sí se retiraron. El cruce del 2026-07-31 (`scripts/buscar-en-excel.mjs`) los había encontrado en su **hoja `SUB N`**, pero esas hojas están desactualizadas: Camilo revisó la lista y confirmó que **ya no están en el club**. Retirados con `scripts/retirar-alumnos.mjs` (14 alumnos, `activo = false`); conservan historial y sus pagos siguen contando al recaudado del año (spec 14). El seed **no los reactiva**: `activo` no va en el `onConflictDoUpdate`.
- **Sin fecha de nacimiento:** ~~15 activos~~ **resuelta (2026-08-07): 0 activos sin fecha.** El cliente revisó la lista de 15 y confirmó que solo `ANGEL SANTIAGO` sigue en el club (`2020-02-26`, cargada con `scripts/fix-fechas-nacimiento.mjs`, sigue en SUB 6); los otros 14 se retiraron. Ningún alumno activo depende ya del fallback por año. Verificable con `npx tsx scripts/listar-sin-fecha.mjs`.
- **Efecto en los conteos:** los activos pasan de **96 a 82**. De las 3 filas de `CATEGORIAS` de alumnos retirados **queda 1**: al 2026-08-22 `MATIAS VIDES VASQUEZ` y `MAXIMILIANO PINTO` ya salieron de la hoja y solo sigue `JOSE ANTONIO LOPEZ` (fila 15). El próximo seed la actualizará pero no la reactivará. <!-- TODO: pedir a Camilo - sacarla de CATEGORIAS para que la hoja refleje el plantel real -->

> **Nota:** las hojas `SUB N` del Excel **no son fuente de verdad de quién está activo** — quedaron con alumnos que ya salieron. La única hoja que lee el seed es `CATEGORIAS`, y el estado activo/retirado vive en la base, no en el Excel.
