...
- **Filas que el seed sigue omitiendo (3):** `GERONIMO ESCORCIA` (nacimiento `2106`), `ABRAHAM PEREZ` (sin nacimiento) y `JUAN PABLO MAESTRE` (sin documento). Las dos primeras ya existen en la base desde el seed anterior; se corrigen en el Excel y se re-corre. La fecha real de `GERONIMO ESCORCIA` (`2016-09-08`, confirmada 2026-07-31) ya está en la base, pero **el Excel sigue con `2106`** → el seed lo seguirá omitiendo hasta que se corrija en la hoja.
...
- **Efecto en los conteos:** los activos pasan de **96 a 82**. Las 3 filas de `CATEGORIAS` de alumnos retirados (`JOSE ANTONIO LOPEZ`, `MATIAS VIDES VASQUEZ`, `MAXIMILIANO PINTO`) siguen en la hoja: el próximo seed las actualizará pero no las reactivará. <!-- TODO: pedir a Camilo - sacarlas de CATEGORIAS para que la hoja refleje el plantel real -->
