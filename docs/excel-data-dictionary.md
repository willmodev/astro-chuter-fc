...
- **Filas que el seed sigue omitiendo (3):** `GERONIMO ESCORCIA` (nacimiento `2106`), `ABRAHAM PEREZ` (sin nacimiento) y `JUAN PABLO MAESTRE` (sin documento). Las dos primeras ya existen en la base desde el seed anterior; se corrigen en el Excel y se re-corre.
- **13 alumnos activos en la base que ya no están en el Excel** (`LIAM GONZALEZ`, `SANTIAGO MARTINEZ`, `JASSIEL` y `JATNIEL MEJIA MORENO`, `SAMUEL ACUÑA`, `CRISTOPHER RODRIGUEZ`, `LIAM RAMOS ARREGOCES`, `JOSUE BALMACEDA BECERRA`, `THIAGO DAZA OLIVEROS`, `DIMAS GUTIERREZ MARQUEZ`, `MATIAS MORENO`, más `GERONIMO ESCORCIA` y `ABRAHAM PEREZ` por las anomalías de arriba). El seed nunca borra, así que siguen activos. <!-- TODO: pedir a Camilo - confirmar si estos 11 se retiraron o solo salieron de la hoja -->
- **Sin fecha de nacimiento (20 activos):** 8 del Excel que traen solo el año + los 12 que ya no figuran en la hoja. Mientras falten, su categoría se calcula por año.
