...
// `hoy` por defecto = ahora: la categoría del roster se evalúa al consultarlo.
export async function listarPlantelCompleto(
  hoy: Date = new Date(),
...
const hoyISO = (): string => formatearFechaISO(new Date());

// Valida categoría (por edad cumplida a `hoy`) y documento único; delega el
// resto al repo.
async function validar(
  ...
  if (categoriaDeFecha(fecha, hoy) === null) {
...
export async function crearAlumno(
  ...
  const fecha = await validar(datos, hoy);
...
export async function editarAlumno(
  ...
  const fecha = await validar(datos, hoy, id);
