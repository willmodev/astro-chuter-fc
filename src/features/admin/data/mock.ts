...
// Sesión ya registrada. Sin imagen (`null`): un object URL no sobrevive la
// recarga, así que la mock solo trae la nota de respaldo.
function ses(
  entrenadorId: string,
  entrenadorNombre: string,
  weekId: string,
  ...
  return { id: `${entrenadorId}-${weekId}-${day}`, entrenadorId, entrenadorNombre, weekId, day, parteCentralImg: null, parteCentralNota, registrado: true, ausentes };
}
...
export const sesiones: Sesion[] = [
  ...
  ses('ent-camilo', 'Camilo Andrade', w1.id, 'Viernes', 'Partido formativo 3v3 en mini-arcos.', []),
  ses('ent-camilo', 'Camilo Andrade', w2.id, 'Lunes', 'Circuito de psicomotricidad con aros y escalera.', []),
  ses('ent-camilo', 'Camilo Andrade', w2.id, 'Miércoles', 'Juegos de persecución con balón y lateralidad.', [9]),
